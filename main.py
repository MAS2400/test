    ...
    def init(self):
        if not os.path.exists(self.path):
            with open(self.path, 'w', newline='', encoding='utf-8') as f:
                csv.writer(f).writerow(["name", "category", "quantity", "price"])  # header
        else:
            # Ensure header includes category; migrate if needed
            try:
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.store = Store(self.cfg.data["csv_path"])  # CSV only
